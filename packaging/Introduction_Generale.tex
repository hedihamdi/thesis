\newpage
\WriteThisInToc\addstarredchapter{Introduction}\chapter*{Introduction}\markboth{\slshape Introduction\hfil}{\slshape Introduction\hfil}

\vskip7mm

\noindent\textbf{\LARGE\sffamily\scshape Les r\'eseaux de r\'egulation g\'en\'etique }

\medskip

\blstbul{15}
		
			\bs\item{\textbf{\sffamily{Bref historique}}}

            Monod, Jacob. Promoteurs. 

  		\bs\item{\textbf{\sffamily{Divers modes de r\'egulation}}}

		\blste{10}
		
			\item{\underline{\sffamily{Promoteurs}}}
		
		
			\medskip
			\item{\underline{\sffamily{Enhancers}}}
		
			\medskip
			\item{\underline{\sffamily{Épigénétique}}}
			
		\elste

\elstbul


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\noindent\textbf{\LARGE\sffamily\scshape Interactions entre les facteurs de transcription et l'ADN.}
 


\newpage
\noindent\textbf{\sffamily\scshape Quelques remarques sur la version pdf du manuscrit}

Voici quelques remarques sur la version pdf de ce manuscrit, qui peuvent rendre la lecture plus aisée. Dans la table des matières, la liste des figures et la liste des annexes, les titres sont des liens hypertexte qui pointent vers l'item décrit. Dans la liste des notations utilisées et la bibliographie, ce sont les numéros de page qui sont des liens hypertexte.

