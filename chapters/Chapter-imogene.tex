...
\subsection{STUBB : une méthode utilisant des motifs connus, les corrélations entre sites de fixations, et la phylogénie}
...
génératif de type modèle de Markov caché (HMM pour \textit{Hidden Markov
Model}). Il est basé sur l'algorithme Ahab~\cite{Rajewsky2002kl} -- lui-même
basé sur l'algorithme MobyDick~\cite{Bussemaker2000ve} --, qui peut être vu
comme une extension de MEME au cas où les séquences contiennent plusieurs sites
de fixations pour différents motifs. Notamment, ces méthodes ont l'intérêt tout
comme MEME de ne pas avoir de seuil arbitraire pour définir un site car elles
moyennent sur toutes les segmentations possibles de la séquence. On parle
notamment de modèle thermodynamique (voir
...
l'algorithme utilise comme condition initiale des PWMs de motifs connus pour
être impliqués dans la co-régulation étudiée. 
...
décrit la génération d'une séquence $S$ de la manière suivante : à chaque pas,
le processus choisit un motif $w_i\in W$ avec une probabilité $p_i$ ou le motif
\textit{background} $w_b$ avec une probabilité $1-\sum_ip_i$.  Une fois le
motif $w$ choisi, une séquence est échantillonnée à partir de la PWM de $w$ et
est ajoutée à la séquence $S$. Le processus est itéré jusqu'à ce que la
séquence générée atteigne une taille $L$.  La séquence de motifs choisis au
cours de la procédure définit une segmentation $T$. La probabilité que la
séquence observée soit générée par ce processus de paramètres $\theta
= \{w_i,p_i\}$ est 
...
Des informations sur les corrélations entre motifs sont introduites
...
Enfin, STUBB utilise des informations provenant de la comparaison
phylogénétique avec des séquences provenant d'autres espèces. Les séquences des
différentes espèces sont d'abord alignées, puis la probabilité de générer
l'alignement est calculée à l'aide d'un modèle phylogénétique. Ce modèle permet
de prendre en compte le fait que les séquences homologues sont corrélées du
fait qu'elles dérivent d'un ancêtre commun. Dans le cas de Stubb, les espèces
sont supposées liées par un topologie étoilée, \cad que les espèces partagent
un seul ancêtre commun. Le modèle d'évolution suppose que les différentes bases
de la séquence évoluent indépendamment, à la même fréquence, et que la
probabilité de fixation d'une mutation $b \to b'$ à la position $i$ est
proportionnelle au poids de la PWM du nucléotide $b$ à cette position. Ce
modèle est identique au modèle que nous introduisons dans l'article (voir
section \ref{sec:article_imogene}), et qui est inspiré du modèle de
Felsenstein~\cite{Felsenstein1981p3782}. La probabilité $P(\sigma|w)$ de
générer l'alignement $\sigma$ de séquences $s$ avec le motif $w$ de taille
$L_w$ s'écrit alors :
...
\apriori donnée par $w$ est identique : l'information phylogénétique permet
dans ce cas de rejeter un \og mauvais \fg modèle qui ne le serait pas si les
séquences orthologues étaient considérées comme indépendantes. Pour des espèces
...
Dans la section précédente, nous avons vu qu'utiliser l'information
phylogénétique permettait de mieux attribuer un site à un motif (cf cas où les
espèces sont proches). Néanmoins, le modèle phylogénétique sous-jacent était
relativement simple. L'algorithme MONKEY~\cite{Moses2004p656} propose
d'utiliser des modèles d'évolution plus complexes pour détecter les sites
...
\begin{equation}
    \mathcal{S}(s) = \log \frac{P(s|w)}{P(s|\pi)} = \sum_i  \log \frac{w_{i,b(i)}}{\pi_b}
\end{equation}

Le but est de généraliser ce score au cas de l'alignement $\sigma$ de séquences
$s$, en réalisant cette fois le calcul sur l'ancêtre commun $A$ des séquences.
L'ancêtre commun, ainsi que tous les ancêtre communs intermédiaires pour une
topologie d'arbre $T$ quelconque, ne sont pas observés et il faut donc sommer
sur tous leurs états (séquences) possibles étant donnés l'alignement observé
$\sigma$, l'arbre $T$ et le modèle d'évolution. La solution générale de ce
problème a été donnée par \citet{Felsenstein1981p3782}. Notamment, nous pouvons
nous concentrer sur le cas à deux espèces, le cas général procédant par
récurrence à partir de ce cas simple. Nous pouvons aussi nous concentrer sur
une position $i$ donnée, puisque les positions sont indépendantes.
...
\begin{equation}
    \mathcal{S}_{\text{cons}}(s_1,s_2) = \log \frac{P(s_1,s_2 | w,T,R_w)}{P(s_1,s_2 | w,T,R_{\text{back}})}
\end{equation}

où $R_w$ et $R_{\text{back}}$ sont des matrices de taux de transition décrivant
le processus de substitution au cours de l'évolution pour le cas d'un site de
fixation du motif $w$ et pour le \textit{background}. Ainsi la transition de la
base $b$ à la base $b'$ s'écrit

\begin{equation}
    p_{b\to b'} = \left(e^{Rd}\right)_{b,b'}
\end{equation}

où $R$ est la matrice de taux de transition et $d$ le temps évolutif. Dans le cas
simple à deux espèces, l'arbre $T$ est en étoile, avec des distances $d_1$ et
$d_2$ de l'ancêtre commun pour les espèces contenant les bases $s_1$ et $s_2$.
Par ailleurs, les espèces évoluent indépendamment depuis leur séparation.
Notant $b$ la base sur l'ancêtre commun et $M$ le modèle (motif ou
\textit{background}) considéré, on a finalement :
...
Le calcul sur un nombre quelconque d'espèces se fait finalement
...
Plusieurs modèles peuvent être choisis pour les matrices de transition. Pour ce
qui est du \textit{background}, il peut être décrit par des modèles neutres.
Par exemple le modèle de Felsenstein \cite{Felsenstein1981p3782} est le modèle
le plus simple dont la distribution d'équilibre redonne les fréquences du
...
mutations, appelées transversions. Ce modèle est utilisé dans MONKEY pour
décrire le \textit{background}. Pour ce qui est du motif, les taux de
...
Pour prendre cette variation en compte, une possibilité est de modifier un
modèle neutre pour utilisant à la place des fréquences \textit{background} les
fréquences données par la PWM : c'est ce qui est fait dans Stubb avec le modèle
Felsenstein dans l'éq.\ref{eq:stubb-felsen}. Dans MONKEY, les auteurs utilisent
un modèle plus complexe, appelé modèle HB, préalablement introduit par Halpern
et Bruno pour étudier l'évolution des régions codantes~\cite{Halpern1998p548}.
Dans ce modèle, le taux de substitution $r^i_{a,b}$ de la base $a$ vers la base
$b$ en position $i$ est à une constante près le produit de la probabilité de
mutation neutre (indépendante de la position) de $a$ vers $b$ par une
probabilité de fixation $f^i_{a,b}$ :

\begin{equation}
    r^i_{a,b} \propto Q_{a,b} f^i_{a,b}
\end{equation}
...
que la population est diploïde), $s$ la valeur adaptative relative de la base
$b$ par rapport à la base $a$ en position $i$, et l'évolution est quasi-neutre
($2Ns\ll1$). On a donc :
...
\begin{equation}
    ...
    \frac{w_{i,b}Q_{b,a}}{w_{i,a}Q_{a,b}} = \frac{f^i_{a,b}}{f^i_{b,a}}
\end{equation}
...
\begin{equation}
    \label{eq:rate_HB}
    \boxed{
    R(i)_{a,b} = Q_{a,b} \frac{x \log x }{x-1}
}
\end{equation}
...
$b$ est très conservée par rapport à la base $a$ ($x>1$), les substitutions de
$a$ vers $b$ sont plus fréquentes que sous le modèle neutre : $R(i)_{a,b}
> Q_{a,b}$.
