...
\section{Analyse thermodynamique des modèles indépendant et avec couplages} 
...
Le cas de l'équation \ref{eq:boltzmann} correspond au cas particulier $\beta=1$
(\cad que le paramètre $\beta$ est incorporé dans les différents
$\beta_\alpha$). Nous voulons voir comment l'amplification ou la diminution
globale de l'écart entre les énergies affecte la possibilité du système
d'explorer les différents états $s$ possibles. À température nulle ($T\to0 $ ou
$\beta \to \infty$), le système reste dans le niveau fondamental de minimum
d'énergie, alors qu'à des températures non nulles le système à l'énergie $E_0$
transite vers un état d'énergie supérieure $E_1$ avec une probabilité
$\exp\left(-\beta (E_1-E_0)\right)$. Lorsqu'un paysage énergétique est composé
de plusieurs puits d'énergie avec des barrières importantes, on s'attend
à avoir une (ou plusieurs) températures critiques pour lesquelles de fortes
différences d'énergie deviennent franchissables. L'énergie moyenne peut alors
être significativement affectée, sautant soudainement à une nouvelle valeur du
fait des poids des nouveaux états explorés. 

La chaleur spécifique permet de caractériser ces sauts soudains d'énergie
moyenne lors de la variation de la température, caractérisant des transitions
de phase. Elle mesure simplement la variation d'énergie moyenne résultant d'une variation de température :
...
corrélations de l'énergie sur l'ensemble des états du système. Nous avons
calculé sa variation en fonction de la température pour les modèles indépendant
et avec dépendances obtenus pour les différents TFs étudiés. L'introduction de
la température se faire de manière fictive en multipliant les énergies par
$\beta$, afin de se placer dans le cadre de l'équation
...
pics de chaleur spécifique pour des températures de l'ordre de $\sim10^{-1}$ et
$\sim 5$ (par exemple, $0.4$ et $2.8$ pour le modèle indépendant de Twist). Il
y a de légères variations entre les deux modèles : notamment, le premier pic
semble renforcé dans plusieurs cas. Néanmoins, le nombre de transitions de
phase (ou de pics) reste le même.
...
(températures) associées, il faut revenir aux modèles d'énergie. Dans le modèle
indépendant, les champs sont simplement le logarithme naturel de la probabilité
d'observer un nucléotide à une position donnée $h_i(a) = -\log P_i(a)$ (la
jauge est choisie telle que $\mathcal{Z}=1$). En valeur absolue, les champs $h$
proches de $0$ correspondent aux nucléotides très conservés (toujours
observés), les valeurs autour de $1$ correspondent à des nucléotides également
observés ($|\log(1/4)| \sim 1.4$) et les valeurs autour de $10$ correspondent
aux nucléotides qui ne sont jamais observés, au pseudocount près (pour un
pseudocount de $1$ et $10^4$ séquences, $|\log(10^{-4})| \sim 9.2$). On
retrouve ces valeurs typiques lorsque l'on regarde l'histogramme des valeurs
absolues de $h$ obtenues dans les modèles indépendant des différents TFs
étudiés (fig.~\ref{fig:histogrammes_champs}A).  On peut maintenant mieux
comprendre les pics de chaleur spécifique. Lorsque la température se rapproche
de $1$ (par le bas), les nucléotides dégénérés d'énergie $h_i\sim1$ deviennent
accessibles, augmentant significativement la valeur de l'énergie moyenne. Puis,
lorsque la température se rapproche de $10$, les nucléotides non observés
d'énergie $h_i\sim10$ deviennent à leur tour accessibles, augmentant à nouveau
l'énergie moyenne.

Dans le cas du modèle de Potts (fig.~\ref{fig:histogrammes_champs}B), on
observe des interactions réparties autour d'un mode centré autour de $J_{i,j}
\sim 0.5$. Ceci explique sans doute le renforcement du premier pic de chaleur
spécifique observé pour plusieurs TFs de la figure
